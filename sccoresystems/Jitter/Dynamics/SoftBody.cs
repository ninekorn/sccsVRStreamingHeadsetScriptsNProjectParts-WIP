...
namespace Jitter.Dynamics
{
    public partial class SoftBody : IBroadphaseEntity
    {
        ...
        public class MassPoint : RigidBody
        {
            ...
            public MassPoint(Shape shape, SoftBody owner, Material material, int rIndex, int rTag)
                : base(shape, material, true, rIndex, rTag)
            {
                this.SoftBody = owner;
            }
        ...
        public SoftBody(int sizeX,int sizeY, float scale, int rIndex, int rTag)
        {
            ...
            AddPointsAndSprings(indices, vertices, rIndex, rTag);
        ...
        public SoftBody(List<TriangleVertexIndices> indices, List<JVector> vertices, int rIndex, int rTag)
        {
            EdgeSprings = springs.AsReadOnly();
            VertexBodies = points.AsReadOnly();
        ...
        private void AddPointsAndSprings(List<TriangleVertexIndices> indices, List<JVector> vertices, int rIndex, int rTag)
        {
            for (int i = 0; i < vertices.Count; i++)
            {
                MassPoint point = new MassPoint(sphere, this,material, rIndex, rTag);
